...
export const createSupplierSchema = Joi.object({
  ...
  lead_time_days: Joi.number().integer().min(0).max(365).default(7),
  payment_terms: Joi.string().max(100).allow('', null),
  reliability_score: Joi.number().min(0).max(1).default(0.85),
  is_active: Joi.boolean().default(true),
});
...
export const updateSupplierSchema = Joi.object({
  ...
  postal_code: Joi.string().max(20).allow('', null),
  lead_time_days: Joi.number().integer().min(0).max(365),
  payment_terms: Joi.string().max(100).allow('', null),
  reliability_score: Joi.number().min(0).max(1),
  is_active: Joi.boolean(),
}).min(1);
