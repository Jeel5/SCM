  ...
  async findAvailableProducts({ organizationId, category, limit = 200 } = {}, client = null) {
    ...
    const query = `
      SELECT
        ...
        p.internal_barcode, p.brand, p.country_of_origin,
  ...
  async update(id, data, client = null) {
    const result = await this.query(
      `UPDATE products SET
         ...
         country_of_origin     = COALESCE($20, country_of_origin),
         warranty_period_days  = COALESCE($21, warranty_period_days),
         shelf_life_days       = COALESCE($22, shelf_life_days),
         tags                  = COALESCE($23, tags),
         supplier_id           = COALESCE($24, supplier_id),
         updated_at            = NOW()
       WHERE id = $25
       RETURNING *`,
      [
